...
def generate_pseudo_images_per_sample():
    ...
    for idx, row in df.iterrows():
        ...
        # 保存伪图（使用样本ID作为文件名）
        image_path = f'pseudo_images/zq-full-h/{sample_id}.png'
...
if __name__ == '__main__':
    # 确保结果目录存在
    os.makedirs('results/zq-full-h', exist_ok=True)
    ...
    # 保存数据集
    os.makedirs('datasets/zq-full-h', exist_ok=True)
    save_dataset(train_dataset, 'datasets/zq-full-h/train_dataset.pth')
    save_dataset(val_dataset, 'datasets/zq-full-h/val_dataset.pth')

    # 保存标签编码器
    os.makedirs('models/zq-full-h', exist_ok=True)
    joblib.dump(label_encoder, 'models/zq-full-h/label_encoder.pkl')
    print("标签编码器已保存到 models/zq-full-h/label_encoder.pkl")

    # 保存标签映射信息
    with open('datasets/zq-full-h/label_mapping.txt', 'w') as f:
        for original, encoded in label_mapping.items():
            f.write(f"{original}% -> {encoded}\n")
    print("标签映射信息已保存到 datasets/zq-full-h/label_mapping.txt")
    ...
    for epoch in range(50):  # 增加训练轮数
        ...
        if val_acc > best_val_acc:
            ...
            torch.save({
                ...
                'epoch': epoch,
                'val_acc': val_acc
            }, 'models/zq-full-h/best_feature_classifier.pth')
    ...
    checkpoint = torch.load('models/zq-full-h/best_feature_classifier.pth')
    ...
    class_names = [f"{c}%" for c in label_encoder.classes_]
    sns.heatmap(conf_mat, annot=True, fmt='d', cmap='Blues',
                xticklabels=class_names, yticklabels=class_names)
    plt.xlabel('预测标签')
    plt.ylabel('真实标签')
    plt.title('混淆矩阵')
    plt.savefig('results/zq-full-h/confusion_matrix.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("混淆矩阵已保存到 results/zq-full-h/confusion_matrix.png")
    ...
    # 保存分类报告
    with open('results/zq-full-h/classification_report.txt', 'w') as f:
        f.write(report)
    print("分类报告已保存到 results/zq-full-h/classification_report.txt")
