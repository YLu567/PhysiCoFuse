...
def load_and_preprocess_data(file_path):
    ...
    # 创建数据数组
    data = np.zeros((n_samples, n_features, n_freqs))  # (样本数, 特征数, 频率数)
    ...
    for sample_idx in range(n_samples):
        for freq_idx, freq in enumerate(FREQUENCIES):
            for feat_idx, feat in enumerate(FEATURES):
                ...
                data[sample_idx, feat_idx, freq_idx] = df.loc[sample_idx, col_name]
...
if __name__ == '__main__':
    ...
    config = {
        ...
        'num_epochs': 50,  # 训练轮数
        'num_classes': 9,  # 类别数
        'save_dir': 'checkpoints/ResNet',  # 模型保存目录
    }
    ...
    try:
        ...
        print(f"数据形状: {data.shape}, 标签形状: {len(labels)}")
    ...
    model = ResNet_6(
        ...
        input_channels=data.shape[1]  # 特征数作为输入通道
    ).to(device)
    ...
    try:
        ...
        print("\n模型摘要:")
        summary(model, input_size=(config['batch_size'], data.shape[1], data.shape[2]))
    except ImportError:
        print("未安装torchinfo，跳过模型摘要")
    ...
    optimizer = optim.SGD(model.parameters(), lr=config['learning_rate'], momentum=0, weight_decay=0.01)

    # 学习率调度器
    scheduler = ReduceLROnPlateau(
        optimizer, mode='max', factor=0.95, patience=10, verbose=True
    )
    ...
    for epoch in range(1, config['num_epochs'] + 1):
        ...
        if test_f1 > best_f1:
            best_f1 = test_f1
            model_path = os.path.join(config['save_dir'], f'best_resnet_epoch{epoch}_f1_{test_f1:.4f}.pth')
            torch.save({
                'epoch': epoch,
    ...
    # 保存最终模型
    final_model_path = os.path.join(config['save_dir'], 'final_resnet_model.pth')
