...
if __name__ == '__main__':
    ...
    for epoch in range(50):
        ...
        if val_acc > best_val_acc:
            ...
            os.makedirs('models/zq', exist_ok=True)
            torch.save(model.state_dict(), 'models/zq/best_feature_classifier.pth')
            print(f"保存最佳模型，验证准确率: {val_acc:.2f}%")

    # 加载最佳模型并做最终评估
    model.load_state_dict(torch.load('models/zq/best_feature_classifier.pth'))
    model.eval()
    ...
    }, 'models/zq/full_model.pth')
    print("完整模型已保存到 models/zq/full_model.pth")